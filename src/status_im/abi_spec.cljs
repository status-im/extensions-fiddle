...
(defn right-pad [x]
  (let [len (count x)
    ...
    (if (= 64 to-pad)
      x
      (.rightPad js/Web3Utils x (+ len to-pad)))))
...
(defn left-pad [x]
  (let [len (count x)
    ...
    (if (= 64 to-pad)
      x
      (.leftPad js/Web3Utils x (+ len to-pad)))))

(defn to-two-complement [x]
  (when x
    (subs (.toTwosComplement js/Web3Utils x) 2)))

(defn from-utf8 [x]
  (when x
    (subs (.fromUtf8 js/Web3Utils x) 2)))

(defn bytes-to-hex [x]
  (when x
    (subs (.bytesToHex js/Web3Utils x) 2)))

(defn number-to-hex [x]
  (when x
    (subs (.numberToHex js/Web3Utils x) 2)))

(defn hex-to-utf8 [x]
  (.hexToUtf8 js/Web3Utils (str "0x" x)))

(defn hex-to-number [x]
  (when x
    (.toNumber (js/toBigNumber (str "0x" x) 16))))

(defn sha3 [s]
  (.sha3 js/Web3Utils (str s)))
